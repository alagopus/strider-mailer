...
describe('Mailer', function () {
  ...
  describe('#elapsed_time()', function () {
    ...
    it('should correctly determine elapsed time less than 60 seconds', function () {
      ...
      var elapsedTime = mailer.elapsed_time(start, finish);
  ...
  describe('#send', function () {
    ...
    it('should execute smtp transport send function when config.smtp set', function (done) {
      ...
      mailer.send('to', 'subject', 'textBody', 'htmlBody', 'from', function (error) {
        expect(error).to.be.ok();
        done();
      });
    });

    it.skip('should execute sendgrid transport send function when config.sendgrid set', function (done) {
